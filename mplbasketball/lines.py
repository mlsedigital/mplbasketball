...
def Lines(xstart, ystart, xend, yend, color=None, n_segments=100, comet=False, transparent=False,
          alpha_start=0.01, alpha_end=1, cmap=None, ax=None, 
          fr: Literal["h", "hl", "hr", "v", "vu", "vd"] = "h",
          to: Literal["h", "hl", "hr", "v", "vu", "vd"] = "h",
          origin: Literal["center", "top-left", "bottom-left", "top-right", "bottom-right"] = "center",
          court_type: Literal["nba", "wnba", "ncaa", "fiba"] = "nba",
          reverse_cmap=False, **kwargs):
    """Plots Lines using matplotlib.collections.LineCollection on basketball courts.
    
    This is a fast way to plot multiple lines without loops.
    Also enables lines that increase in width or opacity by splitting
    the line into n_segments of increasing width or opacity as the line progresses.
    ...
    color : matplotlib color or sequence of colors, optional
    ...
    **kwargs : 
    ...
    >>> court = Court()
    >>> fig, ax = court.draw()
    >>> from mplbasketball.lines import Lines
    >>> Lines(20, 20, 45, 80, comet=True, transparent=True, ax=ax)
    ...
    if 'colors' in kwargs:
        warnings.warn("Lines method takes 'color' as an argument, 'colors' is ignored")
    ...
    if fr != to:
        xstart, ystart = transform(xstart, ystart, fr, to, origin, court_type)
        xend, yend = transform(xend, yend, fr, to, origin, court_type)
    ...
    # Handle transparent effect
    if transparent:
        cmap = _create_transparent_cmap(color, cmap, n_segments, alpha_start, alpha_end)
...
def _create_transparent_cmap(color, cmap, n_segments, alpha_start, alpha_end):
    """Create a transparent colormap for lines."""
    if cmap is None:
        color_rgba = to_rgba_array(color)[0]
        colors = np.array([color_rgba] * n_segments)
        colors[:, 3] = np.linspace(alpha_start, alpha_end, n_segments)
        from matplotlib.colors import ListedColormap
        return ListedColormap(colors)
    else:
        # If cmap is provided, create transparent version
        if isinstance(cmap, str):
            cmap = colormaps.get_cmap(cmap)
...
def _lines_cmap(xstart, ystart, xend, yend, lw=None, cmap=None, ax=None,
    ...
    line_collection = ax.add_collection(line_collection)
    extent = ax.get_ylim()
    court_array = np.linspace(extent[0], extent[1], n_segments)
    line_collection.set_array(court_array)
    return line_collection


class HandlerLines(HandlerLineCollection):
    """Automatically generated by Lines() to allow use of linecollection in legend."""
